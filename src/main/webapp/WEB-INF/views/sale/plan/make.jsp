...
<body>
					...
					<div id="edit-basic" class="tab-pane in active">
						<form class="form-horizontal" action="" method="post">
							...
							<c:if test="${not empty planList}">
								<h4 class="header blue bolder smaller">历史计划项</h4>
								<div class="row">
								<c:forEach items="${planList}" var="plan">
									<div class="col-md-6 col-sm-12 col-xs-12 no-padding-left" id="plan_date-${plan.plan_id}">
									...
									<div class="col-md-6 col-sm-12 col-xs-12 no-padding-left" id="plan_Todo-${plan.plan_id}">
										<div class="form-group">
											<label class="col-sm-3 control-label no-padding-right" 
												for="confirmPassword">计划内容</label>
											<div class="col-sm-9">
												<div class="clearfix">
													<input type="text" id="plan_todo-${plan.plan_id}" value="${plan.plan_todo}" class="col-xs-10">
													<div class="pull-right action-buttons" style="margin-top: 1%">
														<a href='javascript:;' style="text-decoration: none;" class="green" onclick="editPlan(${plan.plan_id})"><i class="icon-ok bigger-110"></i></a>
														<a href='javascript:;' style="text-decoration: none;" class="red" onclick="deletePlan(${plan.plan_id})"><i class="icon-trash bigger-110"></i></a>
													</div>
							...
							<h4 class="header blue bolder smaller">制定计划项</h4>
							<div class="row">
								...
								<div class="col-md-6 col-sm-12 col-xs-12 no-padding-left">
									<div class="form-group">
										...
										<div class="col-sm-9">
											<div class="clearfix">
												<input type="text" id="plan_todo" class="col-xs-10">
											</div>
										</div>
									</div>
								</div>
	...
	function deletePlan(plan_id){
		...
		$.post(url,agrs).done(function(result){
			if (result=="success") {
				$("#plan_date-"+plan_id).remove();
				$("#plan_Todo-"+plan_id).remove();
				Dialog.alert("删除成功!");
			}else {
				Dialog.alert("删除失败!");
			}
	...
	$(function() {
		$("#saveBtn").click(function() {
			...
			var args = {"sale_id":${saleChance.sale_id},"plan_date":plan_date,"plan_todo":plan_todo};
